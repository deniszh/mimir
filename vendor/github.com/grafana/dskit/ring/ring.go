...
type Config struct {
	...
	ConsistencyLevel     string                 `yaml:"read_consistency_level" category:"advanced"`
...
func (cfg *Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.StringVar(&cfg.ConsistencyLevel, prefix+"distributor.consistency-level", "quorum", "Specifies the consistency level for ingesters. Supported values are: quorum, any, relaxed_quorum.")
}
...
func NewWithStoreClientAndStrategy(cfg Config, name, key string, store kv.Client, strategy ReplicationStrategy, reg prometheus.Registerer, logger log.Logger) (*Ring, error) {
	...
	consistencyLevel, err := ConvertConsistencyLevel(cfg.ConsistencyLevel)
	if err != nil {
		return nil, err
	}
...
func (r *Ring) ConsistencyLevel() ConsistencyLevel {
