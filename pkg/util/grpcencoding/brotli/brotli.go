// SPDX-License-Identifier: AGPL-3.0-only
// Provenance-includes-license: Apache-2.0

// Package brotli is a wrapper for using https://github.com/andybalholm/brotli/
// with gRPC.
package brotli

import (
	"errors"
	"io"
	"sync"

	"github.com/andybalholm/brotli"
	"google.golang.org/grpc/encoding"
)

const (
	// Name is the name of the compressor.
	Name = "brotli"
)

type compressor struct {
	name             string
	poolCompressor   sync.Pool
	poolDecompressor sync.Pool
}

type writer struct {
	*brotli.Writer
	pool *sync.Pool
}

type reader struct {
	*brotli.Reader
	pool *sync.Pool
}

func init() {
	encoding.RegisterCompressor(newCompressor())
}

func newCompressor() *compressor {
	c := &compressor{
		name: Name,
	}
	c.poolCompressor.New = func() interface{} {
		newW := brotli.NewWriter(io.Discard)
		return &writer{Writer: newW, pool: &c.poolCompressor}
	}
	return c
}

func (c *compressor) Compress(w io.Writer) (io.WriteCloser, error) {
	z := c.poolCompressor.Get().(*writer)
	z.Writer.Reset(w)
	return z, nil
}

func (c *compressor) Decompress(r io.Reader) (io.Reader, error) {
	z, inPool := c.poolDecompressor.Get().(*reader)
	if !inPool {
		newR := brotli.NewReader(r)
		return &reader{Reader: newR, pool: &c.poolDecompressor}, nil
	}
	err := z.Reset(r)
	return z, err
}

func (c *compressor) Name() string {
	return c.name
}

func (zw *writer) Close() error {
	err := zw.Writer.Close()
	zw.pool.Put(zw)
	return err
}

func (zr *reader) Read(p []byte) (n int, err error) {
	n, err = zr.Reader.Read(p)
	if errors.Is(err, io.EOF) {
		zr.pool.Put(zr)
	}
	return n, err
}
