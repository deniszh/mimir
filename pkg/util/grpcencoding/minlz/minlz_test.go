...
func BenchmarkMinLZCompress(b *testing.B) {
...
func BenchmarkMinLZDecompress(b *testing.B) {
	data := []byte(strings.Repeat("123456789", 1024))
	c := newCompressor()
	var buf bytes.Buffer
	w, err := c.Compress(&buf)
...
func BenchmarkMinLZGRPCCompressionPerf(b *testing.B) {
	data := []byte(strings.Repeat("123456789", 1024))
